...
def series_dnn_diff(model, net, IM, LB, UB, opt='origin', Istar=None):
    # dim = IM.shape
    ...
    for name, m in model.named_children():
        ...
        if isinstance(m, nn.Conv2d):
            print('Start ' + name)
            c_wei1 = net[name + '.weight'].to(torch.float32)
            c_bias1 = net[name + '.bias'].to(torch.float32)
            pad = m.padding
            stride = m.stride
            ...
            print('Start ' + name)
            fc_wei1 = net[name + '.weight'].to(torch.float32)
            fc_bias1 = net[name + '.bias'].to(torch.float32)
